...
<body>
    ...
    <div class="app-container">
        ...
        <aside class="sidebar">
            ...
            <nav class="nav-container">
                ...
                <!-- MENU DATA KARCIS KAPAL (AKTIF) -->
                <a href="<?= site_url('/petugas/daftar-karcis-pemilik-kapal') ?>" class="nav-item active">
                    <div class="nav-icon">
                        <i class="fas fa-ship"></i>
                    </div>
        ...
        <div class="main-content">
            ...
            <div class="top-bar">
                <h1 style="margin: 0; font-size: 24px;">
                    <i class="fas fa-ship me-2" style="color: var(--info);"></i>
                    Daftar Karcis Pemilik Kapal
                </h1>
                <div>
                    <a href="<?= site_url('/petugas/input-kapal') ?>" class="btn-action btn-primary">
                ...
                <div class="stats-container">
                    <div class="stat-card">
                        ...
                        <div class="stat-content">
                            ...
                            <div class="stat-label">Total Karcis Kapal</div>
                        </div>
                    </div>
                    ...
                    <div class="stat-card">
                        ...
                        <div class="stat-content">
                            <div class="stat-value">
                                Rp <?= number_format(array_sum(array_column($karcis, 'harga')), 0, ',', '.') ?>
                            </div>
                    ...
                    <?php if (!empty($karcis)): ?>
                            ...
                            <table class="table table-hover" id="karcisTable">
                                ...
                                <tbody>
                                    ...
                                    <?php foreach ($karcis as $item): ?>
                                        <?php
                                        // Safely get data
                                        $id = $item['id'] ?? $item['id_karcis'] ?? 0;
                                        $no_karcis = $item['no_karcis'] ?? 'KP-' . date('Ymd') . '-' . $id;
                                        $nama_pemilik = $item['nama_pemilik'] ?? '-';
                                        $nama_kapal = $item['nama_kapal'] ?? '-';
                                        $jenis_ikan = $item['jenis_ikan'] ?? 'Campuran';
                                        $berat = $item['berat'] ?? $item['berat_kg'] ?? 0;
                                        $harga = $item['harga'] ?? $item['total_harga'] ?? 0;
                                        $status = $item['status_verifikasi'] ?? 'pending';
                                        $tanggal = $item['created_at'] ?? $item['tanggal_input'] ?? date('Y-m-d');
                                        ?>
                                        <tr>
                                            <td><?= $no++ ?></td>
                                            <td>
                                                <strong><?= $no_karcis ?></strong>
                                            </td>
                                            <td><?= $nama_pemilik ?></td>
                                            <td><?= $nama_kapal ?></td>
                                            <td>
                                                <span class="badge badge-info"><?= $jenis_ikan ?></span>
                                            </td>
                                            <td class="text-end">
                                            ...
                                            <td>
                                                <div class="btn-group-action">
                                                    <button class="btn-icon btn-icon-view" title="Lihat Detail" onclick="viewDetail(<?= $id ?>)">
                                                        <i class="fas fa-eye"></i>
                                                    </button>
                                                    
                                                    <?php if ($status == 'pending'): ?>
                                                        <a href="<?= site_url('/petugas/proses-verifikasi/' . $id) ?>" class="btn-icon btn-icon-verify" title="Verifikasi">
                                                            <i class="fas fa-check-circle"></i>
                                                        </a>
                                                    <?php endif; ?>
                                                    
                                                    <button class="btn-icon btn-icon-edit" title="Edit" onclick="editKarcis(<?= $id ?>)">
                                                        <i class="fas fa-edit"></i>
                                                    </button>
                                                    
                                                    <button class="btn-icon btn-icon-delete" title="Hapus" onclick="deleteKarcis(<?= $id ?>)">
                        ...
                        <div class="mt-3 text-end">
                            <small class="text-muted">
                                ...
                                Menampilkan <?= count($karcis) ?> data karcis kapal
                            </small>
                        </div>
                    <?php else: ?>
                        <!-- Empty State -->
                        <div class="empty-state">
                            ...
                            <h3 class="empty-state-title">Belum ada data karcis kapal</h3>
                            <p class="empty-state-description">
                                Mulai dengan menambahkan karcis kapal pertama Anda
                            </p>
                            <a href="<?= site_url('/petugas/input-kapal') ?>" class="btn-action btn-primary">
                                <i class="fas fa-plus-circle me-1"></i>
                                Tambah Karcis Kapal Pertama
                            </a>
                        </div>
                    <?php endif; ?>
                </div>
    ...
    <script>
            ...
            Swal.fire({
                ...
                didOpen: () => {
                    ...
                    setTimeout(() => {
                        Swal.fire({
                            title: 'Detail Karcis',
                            html: `Fitur detail akan menampilkan informasi lengkap karcis dengan ID: ${id}`,
                            icon: 'info',
        ...
        // Edit karcis function
        function editKarcis(id) {
            Swal.fire({
                title: 'Edit Karcis',
        ...
        // Delete karcis function
        function deleteKarcis(id) {
            Swal.fire({
                title: 'Hapus Karcis?',
                ...
                if (result.isConfirmed) {
                    // In a real application, you would send AJAX request to delete
                    Swal.fire({
                        title: 'Dihapus!',
                        text: 'Data karcis telah dihapus.',
                        icon: 'success',
                        timer: 1500,
                        showConfirmButton: false
                    }).then(() => {
