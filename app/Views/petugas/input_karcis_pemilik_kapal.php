...
<head>
    ...
    <style>
        ...
        * {
        ...
        body, .main-content, .form-container, .form-card, .form-section, .kapal-info, .auto-generate {
...
<body>
    ...
    <div class="app-container">
        ...
        <aside class="sidebar">
            ...
            <nav class="nav-container">
                ...
                <a href="<?= site_url('/petugas/input-kapal') ?>" class="nav-item active">
                    <div class="nav-icon">
        ...
        <div class="main-content">
            ...
            <div class="top-bar">
                <div class="welcome-section">
                    <h1>Input Karcis Kapal</h1>
                    <p>Form input data karcis untuk pemilik kapal | <?= $nama_lengkap ?></p>
                </div>
            ...
            <div class="form-container">
                ...
                <div class="form-card">
                    <div class="form-header">
                        <h2><i class="fas fa-ship me-2"></i> Form Input Karcis Pemilik Kapal</h2>
                        <p>Isi data karcis kapal dengan lengkap dan benar</p>
                    </div>
                    
                    <form id="karcisKapalForm" action="<?= site_url('/petugas/save-kapal') ?>" method="POST">
                        <!-- Auto Generate No Karcis -->
                        <div class="auto-generate mb-4">
                            <div class="row align-items-center">
                                <div class="col-md-8">
                                    <h5 class="mb-1"><i class="fas fa-barcode me-2"></i> Nomor Karcis</h5>
                                    <p class="text-muted mb-0">Nomor karcis sudah digenerate otomatis</p>
                                </div>
                                <div class="col-md-4 text-end">
                                    <button type="button" id="generateNoKarcis" class="generate-btn">
                                        <i class="fas fa-sync-alt me-2"></i> Generate Baru
                                    </button>
                                </div>
                            </div>
                            <div class="mt-3">
                                <input type="text" id="no_karcis" name="no_karcis" class="form-control" 
                                       value="<?= $no_karcis ?>" readonly required>
                                <small class="text-muted">Format: KPL-YYMM-XXX</small>
                            </div>
                        </div>
                        
                        <!-- Section 1: Data Pemilik & Kapal -->
                        <div class="kapal-info">
                            <div class="section-title">
                                <i class="fas fa-user-tie"></i> Data Pemilik & Kapal
                            </div>
                            
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    <label class="form-label">
                                        <i class="fas fa-user"></i> Nama Pemilik Kapal
                                        <span class="required">*</span>
                                    </label>
                                    <input type="text" id="nama_pemilik" name="nama_pemilik" 
                                           class="form-control" placeholder="Contoh: H. Muhammad Arifin" 
                                           value="<?= old('nama_pemilik') ?>" required>
                                </div>
                                
                                <div class="col-md-6 mb-3">
                                    <label class="form-label">
                                        <i class="fas fa-ship"></i> Nama Kapal
                                        <span class="required">*</span>
                                    </label>
                                    <input type="text" id="nama_kapal" name="nama_kapal" 
                                           class="form-control" placeholder="Contoh: KM. Bahagia Jaya" 
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select id="jenis_ikan" name="jenis_ikan" class="form-select" required>
                                        ...
                                        <option value="bawal" <?= old('jenis_ikan') == 'bawal' ? 'selected' : '' ?>>Bawal</option>
                                        <option value="bandeng" <?= old('jenis_ikan') == 'bandeng' ? 'selected' : '' ?>>Bandeng</option>
                                        <option value="teri" <?= old('jenis_ikan') == 'teri' ? 'selected' : '' ?>>Teri</option>
                                        <option value="sarden" <?= old('jenis_ikan') == 'sarden' ? 'selected' : '' ?>>Sarden</option>
                                        ...
                                        <option value="kembung" <?= old('jenis_ikan') == 'kembung' ? 'selected' : '' ?>>Kembung</option>
                                        <option value="selar" <?= old('jenis_ikan') == 'selar' ? 'selected' : '' ?>>Selar</option>
                                        <option value="campuran" <?= old('jenis_ikan') == 'campuran' ? 'selected' : '' ?>>Campuran</option>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <div class="input-group">
                                        <input type="number" id="berat" name="berat" 
                                               class="form-control" step="0.1" min="10" max="10000" 
                                               placeholder="Contoh: 250.5" 
                                               value="<?= old('berat') ?>" required>
                                        <span class="input-group-text">kg</span>
                                    </div>
                                    <small class="text-muted">Berat total ikan dari kapal</small>
                                </div>
                                
                                <div class="col-12 mb-3">
                                    <label class="form-label">
                                        <i class="fas fa-tag"></i> Harga Total (Rp)
                                        <span class="required">*</span>
                                    </label>
                                    <div class="input-group">
                                    ...
                                    <small class="text-muted">Total harga ikan dari kapal</small>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="action-buttons">
                            <button type="submit" class="btn-submit">
                                <i class="fas fa-save me-2"></i> Simpan Karcis Kapal
                            </button>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Generate No Karcis baru
            document.getElementById('generateNoKarcis').addEventListener('click', function() {
                const prefix = 'KPL';
                const year = new Date().getFullYear().toString().slice(-2);
                const month = String(new Date().getMonth() + 1).padStart(2, '0');
                const random = Math.floor(100 + Math.random() * 900);
                
                document.getElementById('no_karcis').value = `${prefix}-${year}${month}-${random}`;
            });
            ...
            document.getElementById('resetForm').addEventListener('click', function(e) {
                ...
                if (confirm('Apakah Anda yakin ingin mereset form? Semua data yang sudah diisi akan hilang.')) {
                    document.getElementById('karcisKapalForm').reset();
                    // Reset tanggal ke hari ini
                    document.getElementById('tanggal').value = new Date().toISOString().split('T')[0];
                    // Regenerate no karcis
                    document.getElementById('generateNoKarcis').click();
                }
            });
            
            // Form validation before submit
            document.getElementById('karcisKapalForm').addEventListener('submit', function(e) {
                // Check required fields
                const requiredFields = ['nama_pemilik', 'nama_kapal', 'tanggal', 'jenis_ikan', 'berat', 'harga'];
            ...
            window.fillExample1 = function() {
                document.getElementById('nama_pemilik').value = 'H. Muhammad Arifin';
                document.getElementById('nama_kapal').value = 'KM. Bahagia Jaya';
                document.getElementById('tanggal').value = '<?= date("Y-m-d") ?>';
                document.getElementById('jenis_ikan').value = 'campuran';
                document.getElementById('berat').value = '350.8';
                document.getElementById('harga').value = '8770000';
            };
            
            window.fillExample2 = function() {
                document.getElementById('nama_pemilik').value = 'Bapak Suroso';
                document.getElementById('nama_kapal').value = 'KM. Sumber Rejeki';
                document.getElementById('tanggal').value = '<?= date("Y-m-d") ?>';
                document.getElementById('jenis_ikan').value = 'tongkol';
                document.getElementById('berat').value = '180.5';
                document.getElementById('harga').value = '4500000';
            };
            
            window.fillExample3 = function() {
                document.getElementById('nama_pemilik').value = 'Ibu Sri Wahyuni';
                document.getElementById('nama_kapal').value = 'KM. Mutiara Laut';
                document.getElementById('tanggal').value = '<?= date("Y-m-d") ?>';
                document.getElementById('jenis_ikan').value = 'tuna';
                document.getElementById('berat').value = '125.3';
                document.getElementById('harga').value = '7500000';
            };
